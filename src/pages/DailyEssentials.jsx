...
export default function DailyEssentials() {
  ...
  const sections = useMemo(() => {
    return GROUPS.map((g) => ({
      ...g,
      // Same catalog order the Women → Tops/Nighty listings use elsewhere
      // on the site (the order those entries are defined in
      // data/products.js) — Firestore's own order isn't guaranteed and
      // tends to land close to alphabetical, which is what was happening
      // here before this sort was added.
      items: sortByCatalogOrder(
        products.filter((p) => p.category === 'women' && p.menuParent === g.menuParent)
      ),
