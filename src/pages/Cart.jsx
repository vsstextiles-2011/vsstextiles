...
export default function Cart() {
  ...
  const [promo, setPromo] = useState(() => {
    try { return JSON.parse(sessionStorage.getItem(PROMO_STORAGE_KEY)) || null } catch { return null }
  })
  ...
  useEffect(() => {
    if (promo) sessionStorage.setItem(PROMO_STORAGE_KEY, JSON.stringify(promo))
                    ...
                    {item.color && (
                      <span className="inline-flex items-center gap-1">
                        ...
                        {(() => {
                          ...
                          return (
                            <span
                              className="inline-block w-3.5 h-3.5 rounded-full border border-ink/20 shrink-0"
                              style={
                                swatch.topHex
