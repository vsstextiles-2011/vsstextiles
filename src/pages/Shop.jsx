...
import { catalogIndexById } from '../utils/catalogOrder.js'
...
export default function Shop() {
  ...
  const products = useMemo(() => {
    ...
    list = [...list]
    // Base order: always the same order the products are defined in
    // data/products.js, regardless of whatever order Firestore happened
    // to return them in. "Popular" (the default) keeps exactly this;
    // every other sort option below re-sorts on top of it by its own
    // criteria.
    list.sort((a, b) => (catalogIndexById.get(a.id) ?? Infinity) - (catalogIndexById.get(b.id) ?? Infinity))
