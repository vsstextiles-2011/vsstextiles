...
import { sortByType } from './typeOrder.js'
...
export function buildMegaMenu(products) {
  ...
  // Items are built up in whatever order products happen to be iterated
  // in (Firestore doesn't guarantee a stable read order), so every
  // column's items -- and each group's flyout children -- get sorted into
  // the same canonical Type order everywhere else in the app uses (see
  // utils/typeOrder.js), instead of being left in arbitrary/insertion
  // order.
  Object.values(menu).forEach((columns) => {
    columns.forEach((column) => {
      column.items = sortByType(column.items, (item) => item.label)
      column.items.forEach((item) => {
        if (item.children) item.children = sortByType(item.children, (c) => c.label)
      })
    })
  })
