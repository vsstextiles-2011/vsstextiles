...
import SlipIcon from '../components/icons/SlipIcon.jsx'
import { useProducts } from '../context/ProductContext.jsx'
import ProductCard from '../components/common/ProductCard.jsx'

// Slips — a dedicated, always-up-to-date landing page pulling every Slip
// product from both categories that carry them (Women, Girls) into one
// place. Same pattern as TShirts.jsx / DailyEssentials.jsx: this reads
// straight from products.js — add a new Slip to either catalog and it
// appears here automatically, no second list to maintain.
const CATEGORY_ORDER = ['women', 'girls']
const CATEGORY_LABELS = { women: "Women's", girls: 'Girls' }
...
export default function Slips() {
  ...
  const sections = useMemo(() => {
    return CATEGORY_ORDER.map((cat) => {
      const items = products.filter((p) => p.category === cat && p.menuParent === 'Slips')
      return { category: cat, label: CATEGORY_LABELS[cat], items, groupSlug: items[0]?.groupSlug || 'slips' }
      ...
      <div className="container-app">
        <div className="mb-6">
          ...
          <p className="text-sm text-ink-soft mt-1">
            Every Slip in the store, for Women and Girls — {totalCount} styles
          </p>
        </div>

        {/* All / Women / Girls tab switcher — kept local to this page since
            it's a fixed, curated cross-category pairing, not the general
            Shop filter panel. */}
            ...
            {visibleSections.map((section) => (
              <div key={section.category}>
                <div className="flex items-center justify-between gap-4 mb-5 pb-2 border-b border-thread">
                  ...
                  >
                    View all {section.label} Slips <ArrowRight size={14} />
                  </Link>
                </div>
