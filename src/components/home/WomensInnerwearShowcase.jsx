...
//
// Which styles show up in each row, and in what order, is fully
// admin-controlled from that row's own tab in the Admin sidebar (Bras,
// Panties, Slips, Tights, Tops each have their own tab now, grouped under
// "Homepage Sections") — reorder or add/remove a style there and it
// updates here immediately, live from Firestore. No code-array editing
// needed. Both sides share the same STOREFRONT_SECTIONS config
// (utils/storefrontSections.js) so they can never disagree on eligibility.
const INNERWEAR_SECTION_IDS = ['bras', 'panties', 'slips', 'tights']
...
// right below Nighty, using the same row/grid treatment.
const TOPS_SECTION_ID = 'tops'

function buildGroup(products, sectionId) {
  const section = STOREFRONT_SECTIONS.find((s) => s.id === sectionId)
  return { label: section.label, styles: sectionProducts(section, products) }
}
...
export default function WomensInnerwearShowcase() {
  ...
  const groups = useMemo(
    () => INNERWEAR_SECTION_IDS.map((id) => buildGroup(products, id)).filter((g) => g.styles.length > 0),
    [products]
  )
  const topsGroup = useMemo(() => {
    const g = buildGroup(products, TOPS_SECTION_ID)
  ...
  return (
    <>
      ...
      {/* Nighty is Women Outerwear, not Innerwear — kept as its own
          ...
          src/components/home/NightyShowcase.jsx). */}
      <NightyShowcase />
