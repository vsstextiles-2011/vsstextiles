...
import { displayOrder } from '../utils/catalogOrder.js'
...
function typeOptionsFor(list) {
  ...
  const disambiguated = options.map((o) => ({
    slug: o.slug,
    count: o.count,
    label: labelCounts.get(o.label) > 1 && o.menuParent ? `${o.menuParent} ${o.label}` : o.label,
  }))

  return sortByType(disambiguated, (o) => o.label)
}

export default function Shop() {
  const { storeProducts: allProducts } = useProducts()
  ...
  const baseList = useMemo(() => {
    ...
    return list
  }, [allProducts, category, entryHeading, isTopsEntry, search])
  ...
  const products = useMemo(() => {
    ...
    list = [...list]
    // Base order: the product's manual position if an admin has moved it
    // with Inventory's move up/down buttons, otherwise the order products
    // are defined in data/products.js, regardless of whatever order
    // Firestore happened to return them in. "Popular" (the default) keeps
    // exactly this; every other sort option below re-sorts on top of it by
    // its own criteria.
    list.sort((a, b) => displayOrder(a) - displayOrder(b))
...
function FilterPanel({
  ...
  const { storeProducts: allProductsForMenu } = useProducts()
