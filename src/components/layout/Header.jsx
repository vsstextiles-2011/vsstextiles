...
export default function Header() {
  ...
  const { user } = useAuth()
  // Anonymous guest sessions (see AuthContext) are a real Firebase user
  // but not an account -- only route to the profile page for someone
  // who's actually signed up/signed in.
  const isSignedIn = Boolean(user && !user.isAnonymous)
  const accountTo = isSignedIn ? '/profile' : '/account'
  const accountLabel = isSignedIn ? (user.displayName?.split(' ')[0] || 'Profile') : 'Login'
    ...
    >
      <div className="container-app py-3.5 md:py-4 flex items-center gap-4 md:gap-8">
        ...
        <div className="flex items-center gap-4 sm:gap-6 md:pl-6 md:border-l md:border-thread ml-auto md:ml-0">
          ...
          <Link to={accountTo} className="hidden sm:flex flex-col items-center text-ink-soft hover:text-brand transition-colors">
            <User size={22} />
            <span className="text-[11px] mt-0.5 max-w-[64px] truncate">{accountLabel}</span>
          </Link>
