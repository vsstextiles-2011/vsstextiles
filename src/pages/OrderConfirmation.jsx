...
export default function OrderConfirmation() {
  ...
  // Orders now load from a live Firestore listener (see OrderContext),
  // which takes a moment on first mount -- don't redirect away just
  // because the order isn't in the list *yet*. Only treat it as
  // genuinely missing once the listener has actually finished loading.
  if (isLoading) {
    return (
      ...
      <div className="container-app max-w-3xl">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 sm:p-8">
          ...
          <div className="space-y-2 text-sm text-ink-soft max-w-xs ml-auto">
            ...
            {(order.totals.discount - (order.totals.bigOrderDiscount || 0)) > 0 && (
              <div className="flex justify-between text-brand">
                <span>Offer{order.totals.promoCode ? ` (${order.totals.promoCode})` : ''}</span>
                <span>−{formatPrice(order.totals.discount - (order.totals.bigOrderDiscount || 0))}</span>
              </div>
            )}
            {order.totals.bigOrderDiscount > 0 && (
              <div className="flex justify-between text-brand">
                <span>Special Offer (25% off)</span>
                <span>−{formatPrice(order.totals.bigOrderDiscount)}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span>Shipping</span>
