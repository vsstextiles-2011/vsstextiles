...
export default function OffersSection() {
  ...
  // Reads from the same live `isOffer` flags (set per-category in the
  // Admin panel's "Special Offers" tabs) as the dedicated Offers page
  // (src/pages/Offers.jsx) — see OFFER_SECTIONS/sectionProducts in
  // utils/storefrontSections.js.
  const offerSections = useMemo(
    () =>
      OFFER_SECTIONS.map((s) => ({
        category: s.scope,
        label: s.label,
        items: sectionProducts(s, products).slice(0, PRODUCTS_PER_ROW),
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-white/90 text-sm max-w-md">
                Use code <span className="font-mono font-semibold">VSS10</span> at checkout. Valid on all men's, women's, boys' and girls' collections this week only.
              </p>
            </div>
          </div>
          <Link
